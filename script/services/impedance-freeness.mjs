...
/**
 * Takes activities generated by PAE and checks for competing processes.
 * A competing process exists when two or more activities share an arc
 * whose total usage across those activities exceeds its L-value.
 *
 ...
 *   competingActivityIds: number[][],  // pairs/groups of competing process IDs
 *   arcUsage: { arcUID: number, L: number, usedByProcessIds: number[] }[]
 * }}
 */
export function checkCompetingProcesses(activities, simpleModel) {
  // Build arcUID → L map from simpleModel
  const arcLMap = new Map();
  for (const arc of simpleModel.arcs) {
    arcLMap.set(arc.uid, arc.L ?? 1);
  }

  // Count how many activities use each arcUID, and which process IDs
  const arcUsageMap = new Map(); // arcUID → { L, usedByProcessIds: number[] }

  for (const activity of activities) {
    const seen = new Set(); // avoid double-counting same arc in same activity
    for (const arcSet of Object.values(activity.activityProfile)) {
      for (const arcUID of arcSet) {
        ...
        if (!arcUsageMap.has(arcUID)) {
          arcUsageMap.set(arcUID, {
            arcUID,
            L: arcLMap.get(arcUID) ?? 1,
            usedByProcessIds: [],
          });
        }
  ...
  // Find arcs where usage > L (competing)
  const competingArcs = [...arcUsageMap.values()].filter(
    ({ L, usedByProcessIds }) => usedByProcessIds.length > L
  );

  // Collect all process ID pairs that are competing
  const competingActivityIds = competingArcs.map(({ usedByProcessIds }) => usedByProcessIds);

  // Build a competitionLog-shaped array for pae.mjs to merge into its own log
  const competitionLog = competingArcs.map(({ arcUID, arcL, usedByProcessIds }) => ({
    arcUID,
    arcL,
    usedByProcessIds,
    ...
    reason:
      `Arc uid=${arcUID} (L=${arcL}) used by ${usedByProcessIds.length} activities ` +
