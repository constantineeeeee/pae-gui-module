/*
Manager Module for the Traversal Tree Subworkspace
*/
...
// ─── Layout constants ─────────────────────────────────────────────────────────
/** Pixels of padding left of the first column. */
const LEFT_PAD = 80;
/** Pixels of padding above the first row. */
const TOP_PAD  = 80;
/** Horizontal spacing (px) between depth levels. */
const X_GAP    = 280;
/** Vertical spacing (px) between sibling nodes. */
const Y_GAP    = 120;
...
/**
 * Converts a D3-hierarchy tree node (wrapping an NTT node) into the recursive
 * `{ id, ref, children }` structure that `d3.hierarchy` expects.
 *
 * @param {object}            node        - NTT node
 * @param {Map<string, object[]>} childrenOf  - spanning-tree children map
 * @returns {D3TreeNode}
 */
function toTree(node, childrenOf) {
    return {
        id:       node.id,
        ref:      node,
        children: (childrenOf.get(node.id) ?? []).map(c => toTree(c, childrenOf)),
    };
}
...
/**
 * Manages a single traversal-tree subworkspace tab.
 *
 * Lifecycle:
 *  1. Constructed with an application context and a visual-model snapshot.
 *  2. `#initialize` (called from constructor) adds the subworkspace DOM tab,
 *     wires toolbar events, and triggers the first render.
 *  3. `#runAndRender` runs NTT and delegates to `#renderTree` / `#renderResults`.
 *  4. The tab stays alive until the user closes it or navigates away.
 */
export default class TraversalTreeViewerManager {
     ...
     * @param {object|null} visualModelSnapshot    - pre-captured model snapshot (or null to capture live)
     * @param {string[]|null} groupColors          - optional color overrides
     */
    ...
    /**
     * Creates the subworkspace DOM tab, queries key elements, wires all
     * toolbar actions, and triggers the first render.
     *
     * @returns {Promise<void>}
     */
    async #initialize() {
        this.#subworkspace =
            await this.context.managers.workspace.addTraversalTreeSubworkspace(this.id);
    ...
    /**
     * Serializes the current SVG to a PNG file and triggers a browser download.
     * Crops to the tight bounding box of all drawn content and applies a 2×
     * scale for sharper output.
     */
    #exportImage() {
    ...
    /**
     * Runs the NTT algorithm on the current snapshot and re-renders the SVG
     * and sidebar from scratch.
     */
    #runAndRender() {
        const snapshot = this.#snapshot ?? this.context.managers.visualModel.makeCopy();
    ...
    /** Wipes the SVG canvas. */
    #clearSVG() {
        if (this.#svg) this.#svg.innerHTML = "";
    }

    /**
     * Renders a plain-text fallback message into the SVG canvas.
     *
     * @param {string} text
     */
    #drawMessage(text) {
    ...
    /**
     * Formats an S-sequence array as `S([0, a, (ε, m), ε])`.
     *
    ...
    /**
     * Returns true when `node` is a join placeholder — rendered as "(v)".
     *
     * @param {object} node  - NTT node
     * @returns {boolean}
     */
    ...
    /**
     * Draws a labeled node: vertex name (underlined, bold) and S([...])
     * subscript below it.
     *
     * @param {SVGGElement}         g  - parent SVG group
     * @param {number}              x
     * @param {number}              y
     * @param {string}              v  - vertex id label
     * @param {(string | number)[]} S  - S-sequence
     * @returns {NodeBounds}
     */
    ...
    /**
     * Draws a placeholder (join/proxy) node as the text "(v)".
     *
     * @param {SVGGElement} g  - parent SVG group
     * @param {number}      x
     * @param {number}      y
    ...
    /**
     * Appends an SVG line (and optional `i=N` label) to `edgeG`.
     *
     * @param {SVGGElement}  edgeG
     * @param {number}       x1
     * @param {number}       y1
     * @param {number}       x2
     * @param {number}       y2
     * @param {number|null}  label    - time-step label shown as "i=N"; omit to suppress
     * @param {{
     *   stroke?:      string,
     *   strokeWidth?: string,
     *   opacity?:     string,
    ...
    /**
     * Resolves a per-path color index for every maximal path in `res`.
     *
     * @param {{ maximalPaths: object[] }} res  - NTT result
     * @returns {number[]}  color indices (one per maximal path)
     */
    ...
    /**
     * Builds and draws the annotated traversal-tree SVG using D3 for Y
     * placement and a col-rank system for X placement.
     *
     * @param {{ allNodes: object[], maximalPaths: object[], joinTypes: Map }} res
     */
    #renderTree(res) {
        const d3 = window.d3;
        if (!d3) {
        ...
        const superRoot = {
            id:       "__root__",
            ref:      null,
            children: roots.map(r => toTree(r, childrenOf)),
        };
    ...
    /** Resizes the displayed SVG according to the current `#zoom` level. */
    #applyZoom() {
    ...
    /**
     * Sets the zoom level, clamped to `[#zoomMin, #zoomMax]`. When `pivot` is
     * provided as `{ clientX, clientY }` the point under the cursor stays
     * fixed (cursor-anchored zoom, like a typical map or IDE zoom).
     *
     * @param {number}                   z
    ...
    /** Fits the entire tree into the visible drawing area. */
    #zoomFit() {
        const scrollHost = this.#root?.querySelector("[data-tt-drawing]");
    ...
    /**
     * Applies the current `#highlightedPathIdx` to both the SVG edges and the
     * sidebar row chips. Call after toggling `#highlightedPathIdx`.
     */
    #applyPathHighlight() {
        if (!this.#svg) return;
    ...
    /**
     * Populates the sidebar with the maximal-path list (grouped by completion
     * time) and fills the metadata fields (source, sink, node count, time
     * range). Resets any stale highlight state before rebuilding.
     *
     * @param {{ allNodes: object[], maximalPaths: object[] }} res
     */
    #renderResults(res) {
        ...
        if (res.maximalPaths && res.maximalPaths.length > 0) {
            ...
            list.style.cssText = "display:flex;flex-direction:column;gap:10px;font-size:12px;";
            ...
            sortedTimes.forEach((t) => {
                ...
                bucket.forEach(({ leaf, pathIndex }) => {
                    ...
                    labelEl.textContent = `Path ${pathIndex + 1}: ${leaf.v}  —  ${this.#sToString(leaf.S)}`;
